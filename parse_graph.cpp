...
uint parse_graph::parse(
	...
	while( std::getline( inFile, line ) ) {
		...
		{ //This is just a block
			...
			***********************/
			edge_triple edge;
			edge.source = srcVertexIndex;
			edge.destination = dstVertexIndex;
	...
	if (orderSeq == Source) {
        ...
        [](const edge_triple & a, const edge_triple & b)
        {
        ...
        [](const edge_triple & a, const edge_triple & b)
        {
         return a.destination == b.destination? a.source < b.source : a.destination < b.destination ;
        });
