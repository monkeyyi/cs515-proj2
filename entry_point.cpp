...
// enum OrderSeq {Source, Destination};
enum OrderSeq orderSeq;
...
int main( int argc, char** argv )
{
	...
	std::string usage =
		"\tRequired command line arguments:\n\
			...
			Edge list Order: E.g., --order source, or destination\n	";
	...
	try {
		...
		for( int iii = 1; iii < argc; ++iii )
           ...
           exit;
			...
			else if ( !strcmp(argv[iii], "--order") && iii != argc-1 ) {
				if ( !strcmp(argv[iii+1], "source") )
				        orderSeq = OrderSeq::Source;
				else if ( !strcmp(argv[iii+1], "destination") )
    				        orderSeq = OrderSeq::Destination;
				else{
           std::cerr << "\n Un-recognized order for edge list \n\n";
           exit;
         }   
			}
      ...
      exit;
		...
		for (int i=0; i<edgeList.size(); i++) {
			std::cout << "source: "<<edgeList[i].source<<" destination: "<< edgeList[i].destination<<" weight: "<< edgeList[i].weight<< "\n";
		}
		...
		case ProcessingType::Push:
		    puller(&parsedGraph, bsize, bcount); //, smemMethod, syncMethod, orderSeq );
		    break;
